      ...
      in
      {
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            ...
            export MYSQL_SOCK="/tmp/bridge-test-mysql.sock"
            export MYSQL2_SOCK="/tmp/bridge-test-mysql2.sock"
            export MARIA_DIR="$SEED_DIR/mariadb"
            export MARIA2_DIR="$SEED_DIR/mariadb2"
            export MARIA_SOCK="/tmp/bridge-test-maria.sock"
            export MARIA2_SOCK="/tmp/bridge-test-maria2.sock"
            ...
            # MySQL (primary)
            if ! mysqladmin ping -u root --socket="$MYSQL_SOCK" --silent 2>/dev/null; then
              if [ ! -d "$MYSQL_DIR/mysql" ]; then
                rm -rf "$MYSQL_DIR"
                mkdir -p "$MYSQL_DIR"
                mysqld --initialize-insecure --datadir="$MYSQL_DIR" > /dev/null 2>&1
              fi
              mysqld --datadir="$MYSQL_DIR" --socket="$MYSQL_SOCK" --port=3306 --bind-address=127.0.0.1 --daemonize > /dev/null 2>&1
              sleep 2
              if mysqladmin ping -u root --socket="$MYSQL_SOCK" --silent 2>/dev/null; then
                mysql -u root --socket="$MYSQL_SOCK" -e "CREATE DATABASE IF NOT EXISTS testdb;" 2>/dev/null
            ...
            # MySQL (secondary)
            if ! mysqladmin ping -u root --socket="$MYSQL2_SOCK" --silent 2>/dev/null; then
              if [ ! -d "$MYSQL2_DIR/mysql" ]; then
                rm -rf "$MYSQL2_DIR"
                mkdir -p "$MYSQL2_DIR"
                mysqld --initialize-insecure --datadir="$MYSQL2_DIR" > /dev/null 2>&1
              fi
              mysqld --datadir="$MYSQL2_DIR" --socket="$MYSQL2_SOCK" --port=3307 --bind-address=127.0.0.1 --daemonize > /dev/null 2>&1
              sleep 2
              if mysqladmin ping -u root --socket="$MYSQL2_SOCK" --silent 2>/dev/null; then
                mysql -u root --socket="$MYSQL2_SOCK" -e "CREATE DATABASE IF NOT EXISTS testdb;" 2>/dev/null
            ...
            # MariaDB (primary)
            if ! mysqladmin ping -u root --socket="$MARIA_SOCK" --silent 2>/dev/null; then
              if [ ! -d "$MARIA_DIR/mysql" ]; then
                rm -rf "$MARIA_DIR"
                mkdir -p "$MARIA_DIR"
                mysql_install_db --datadir="$MARIA_DIR" --auth-root-authentication-method=normal > /dev/null 2>&1
              fi
              mysqld --defaults-file=/dev/null --datadir="$MARIA_DIR" --socket="$MARIA_SOCK" --port=3308 --bind-address=127.0.0.1 --daemonize > /dev/null 2>&1
              sleep 2
              if mysqladmin ping -u root --socket="$MARIA_SOCK" --silent 2>/dev/null; then
                mysql -u root --socket="$MARIA_SOCK" -e "CREATE DATABASE IF NOT EXISTS testdb;" 2>/dev/null
            ...
            # MariaDB (secondary)
            if ! mysqladmin ping -u root --socket="$MARIA2_SOCK" --silent 2>/dev/null; then
              if [ ! -d "$MARIA2_DIR/mysql" ]; then
                rm -rf "$MARIA2_DIR"
                mkdir -p "$MARIA2_DIR"
                mysql_install_db --datadir="$MARIA2_DIR" --auth-root-authentication-method=normal > /dev/null 2>&1
              fi
              mysqld --defaults-file=/dev/null --datadir="$MARIA2_DIR" --socket="$MARIA2_SOCK" --port=3309 --bind-address=127.0.0.1 --daemonize > /dev/null 2>&1
              sleep 2
              if mysqladmin ping -u root --socket="$MARIA2_SOCK" --silent 2>/dev/null; then
                mysql -u root --socket="$MARIA2_SOCK" -e "CREATE DATABASE IF NOT EXISTS testdb;" 2>/dev/null
            ...
            if ! cockroach sql --host=localhost --port=26257 --insecure -e "SELECT 1" > /dev/null 2>&1; then
              mkdir -p "$CRDB_DIR"
              cockroach start-single-node --insecure --store=path="$CRDB_DIR" --listen-addr=localhost:26257 --http-addr=localhost:8080 --background > /dev/null 2>&1
              sleep 2
            ...
            if ! cockroach sql --host=localhost --port=26258 --insecure -e "SELECT 1" > /dev/null 2>&1; then
              mkdir -p "$CRDB2_DIR"
              cockroach start-single-node --insecure --store=path="$CRDB2_DIR" --listen-addr=localhost:26258 --http-addr=localhost:8081 --background > /dev/null 2>&1
              sleep 2
            ...
            # === MySQL ===
            if mysqladmin ping -u root --socket="$MYSQL_SOCK" --silent 2>/dev/null; then
              mysql -u root --socket="$MYSQL_SOCK" testdb -e "CREATE TABLE IF NOT EXISTS users (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(100) NOT NULL, email VARCHAR(255) UNIQUE NOT NULL, age INT);" 2>/dev/null
              mysql -u root --socket="$MYSQL_SOCK" testdb -e "CREATE TABLE IF NOT EXISTS products (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(100) NOT NULL, category VARCHAR(50) NOT NULL, price DECIMAL(10,2) NOT NULL, stock INT DEFAULT 0);" 2>/dev/null
              mysql -u root --socket="$MYSQL_SOCK" testdb -e "CREATE TABLE IF NOT EXISTS orders (id INT AUTO_INCREMENT PRIMARY KEY, user_id INT, product_id INT, quantity INT NOT NULL DEFAULT 1, total_price DECIMAL(10,2), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (user_id) REFERENCES users(id), FOREIGN KEY (product_id) REFERENCES products(id));" 2>/dev/null
              mysql -u root --socket="$MYSQL_SOCK" testdb -e "INSERT IGNORE INTO users (name, email, age) VALUES ('Alice Johnson','alice@example.com',30), ('Bob Smith','bob@example.com',25), ('Charlie Brown','charlie@example.com',35), ('Diana Prince','diana@example.com',28), ('Eve Adams','eve@example.com',22), ('Frank Miller','frank@example.com',40), ('Grace Lee','grace@example.com',33), ('Henry Wilson','henry@example.com',27), ('Irene Davis','irene@example.com',31), ('Jack Taylor','jack@example.com',29), ('Karen White','karen@example.com',36), ('Leo Garcia','leo@example.com',24), ('Mia Martinez','mia@example.com',26), ('Nathan Clark','nathan@example.com',38), ('Olivia Lewis','olivia@example.com',23), ('Paul Walker','paul@example.com',34), ('Quinn Hall','quinn@example.com',21), ('Rachel Young','rachel@example.com',32), ('Sam King','sam@example.com',37), ('Tina Wright','tina@example.com',28), ('Uma Scott','uma@example.com',26), ('Victor Green','victor@example.com',41), ('Wendy Baker','wendy@example.com',30), ('Xavier Adams','xavier@example.com',35), ('Yara Collins','yara@example.com',29);" 2>/dev/null
            ...
            # === MariaDB ===
            if mysqladmin ping -u root --socket="$MARIA_SOCK" --silent 2>/dev/null; then
              mysql -u root --socket="$MARIA_SOCK" testdb -e "CREATE TABLE IF NOT EXISTS users (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(100) NOT NULL, email VARCHAR(255) UNIQUE NOT NULL, age INT);" 2>/dev/null
              mysql -u root --socket="$MARIA_SOCK" testdb -e "CREATE TABLE IF NOT EXISTS products (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(100) NOT NULL, category VARCHAR(50) NOT NULL, price DECIMAL(10,2) NOT NULL, stock INT DEFAULT 0);" 2>/dev/null
              mysql -u root --socket="$MARIA_SOCK" testdb -e "CREATE TABLE IF NOT EXISTS orders (id INT AUTO_INCREMENT PRIMARY KEY, user_id INT, product_id INT, quantity INT NOT NULL DEFAULT 1, total_price DECIMAL(10,2), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (user_id) REFERENCES users(id), FOREIGN KEY (product_id) REFERENCES products(id));" 2>/dev/null
              mysql -u root --socket="$MARIA_SOCK" testdb -e "INSERT IGNORE INTO users (name, email, age) VALUES ('Alice Johnson','alice@example.com',30), ('Bob Smith','bob@example.com',25), ('Charlie Brown','charlie@example.com',35), ('Diana Prince','diana@example.com',28), ('Eve Adams','eve@example.com',22), ('Frank Miller','frank@example.com',40), ('Grace Lee','grace@example.com',33), ('Henry Wilson','henry@example.com',27), ('Irene Davis','irene@example.com',31), ('Jack Taylor','jack@example.com',29), ('Karen White','karen@example.com',36), ('Leo Garcia','leo@example.com',24), ('Mia Martinez','mia@example.com',26), ('Nathan Clark','nathan@example.com',38), ('Olivia Lewis','olivia@example.com',23), ('Paul Walker','paul@example.com',34), ('Quinn Hall','quinn@example.com',21), ('Rachel Young','rachel@example.com',32), ('Sam King','sam@example.com',37), ('Tina Wright','tina@example.com',28), ('Uma Scott','uma@example.com',26), ('Victor Green','victor@example.com',41), ('Wendy Baker','wendy@example.com',30), ('Xavier Adams','xavier@example.com',35), ('Yara Collins','yara@example.com',29);" 2>/dev/null
              mysql -u root --socket="$MARIA_SOCK" testdb -e "INSERT IGNORE INTO products (name, category, price, stock) VALUES ('Laptop','Electronics',999.99,50), ('Wireless Mouse','Electronics',29.99,200), ('Mechanical Keyboard','Electronics',79.99,150), ('Monitor 27 inch','Electronics',399.99,30), ('Noise-Cancelling Headphones','Electronics',149.99,75), ('Office Chair','Furniture',249.99,40), ('Standing Desk','Furniture',499.99,20), ('Desk Lamp','Furniture',39.99,120), ('Bookshelf','Furniture',129.99,55), ('Filing Cabinet','Furniture',89.99,65), ('Python Cookbook','Books',44.99,300), ('Design Patterns','Books',54.99,250), ('Clean Code','Books',39.99,400), ('The Pragmatic Programmer','Books',49.99,180), ('Refactoring','Books',42.99,220), ('Running Shoes','Sports',119.99,90), ('Yoga Mat','Sports',24.99,500), ('Dumbbell Set','Sports',79.99,60), ('Resistance Bands','Sports',19.99,800), ('Jump Rope','Sports',14.99,650), ('Coffee Maker','Kitchen',89.99,110), ('Blender','Kitchen',59.99,95), ('Toaster','Kitchen',34.99,130), ('Knife Set','Kitchen',69.99,70), ('Water Bottle','Kitchen',12.99,1000);" 2>/dev/null
              mysql -u root --socket="$MARIA_SOCK" testdb -e "INSERT IGNORE INTO orders (user_id, product_id, quantity, total_price) VALUES (1,1,1,999.99), (1,3,1,79.99), (2,2,2,59.98), (3,5,1,149.99), (4,4,1,399.99), (5,2,3,89.97), (5,3,1,79.99), (6,6,1,249.99), (7,7,1,499.99), (8,8,2,79.98), (9,11,1,44.99), (9,12,1,54.99), (10,16,1,119.99), (11,17,3,74.97), (12,21,1,89.99), (13,22,1,59.99), (14,13,2,79.98), (15,9,1,129.99), (16,14,1,49.99), (17,18,2,39.98), (18,10,1,89.99), (19,15,1,42.99), (20,19,4,79.96), (21,23,1,34.99), (22,20,2,29.98), (23,24,1,69.99), (24,25,5,64.95), (25,1,1,999.99), (1,5,2,299.98), (3,11,1,44.99), (6,14,1,49.99), (8,17,1,24.99), (10,21,1,89.99);" 2>/dev/null
