...
// dryRunProvider wraps a PostgresProvider and intercepts writes, logging them
// without executing any operations against the destination.
type dryRunProvider struct {
	inner *PostgresProvider
}

func (d *dryRunProvider) Name() string { return "postgres" }
...
func (d *dryRunProvider) Writer(_ context.Context, opts provider.WriteOptions) provider.Writer {
	return &dryRunWriter{
		opts: opts,
		log:  logger.L().With("component", "postgres-dryrun-writer"),
	}
}
