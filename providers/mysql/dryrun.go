...
// dryRunProvider wraps a MySQLProvider and intercepts writes, logging them
// without executing any operations against the destination.
type dryRunProvider struct {
	inner *MySQLProvider
}

func (d *dryRunProvider) Name() string { return "mysql" }
...
func (d *dryRunProvider) Writer(_ context.Context, opts provider.WriteOptions) provider.Writer {
	return &dryRunWriter{
		opts: opts,
		log:  logger.L().With("component", "mysql-dryrun-writer"),
	}
}
