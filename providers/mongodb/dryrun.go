...
// dryRunProvider wraps a MongoDBProvider and intercepts writes, logging them
// without executing any operations against the destination.
type dryRunProvider struct {
	inner *MongoDBProvider
}

func (d *dryRunProvider) Name() string { return "mongodb" }
...
func (d *dryRunProvider) Writer(_ context.Context, opts provider.WriteOptions) provider.Writer {
	return &dryRunWriter{
		opts: opts,
		log:  logger.L().With("component", "mongodb-dryrun-writer"),
	}
}
